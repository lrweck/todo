...
import (
	...
	"github.com/lrweck/todo/internal/rest"
	"github.com/lrweck/todo/internal/rest/resttesting"
)
...
func TestTasks_Delete(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			"ERR: 404",
			func(s *resttesting.FakeTaskService) {
				s.DeleteReturns(internal.NewErrorf(internal.ErrCodeNotFound, "not found"))
			},
			output{
				http.StatusNotFound,
				&struct{}{},
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			res := doRequest(router,
				httptest.NewRequest(http.MethodDelete, "/task/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee", nil))
...
func TestTasks_Read(t *testing.T) {
		...
		},
		{
			...
			func(s *resttesting.FakeTaskService) {
				s.TaskReturns(internal.Task{},
					internal.NewErrorf(internal.ErrCodeNotFound, "not found"))
			},
			output{
				http.StatusNotFound,
				&rest.ErrorResponse{
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			res := doRequest(router,
				httptest.NewRequest(http.MethodGet, "/task/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee", nil))
...
func TestTasks_Update(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			"ERR: 404",
			func(s *resttesting.FakeTaskService) {
				s.UpdateReturns(internal.NewErrorf(internal.ErrCodeNotFound, "not found"))
			},
			func() []byte {
				b, _ := json.Marshal(&rest.UpdateTasksRequest{
					Description: "update task",
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			res := doRequest(router,
				httptest.NewRequest(http.MethodPut, "/task/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee", bytes.NewReader(tt.input)))
